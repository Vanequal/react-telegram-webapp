...
const AnswerThread = ({ answer, isNew, sectionKey, themeId, isFirstAnswer = false }) => {
  ...
  const handleAnswerReaction = useCallback((reaction) => {
      ...
      <div className={`question-comment-thread ${isFirst ? 'first-comment' : ''}`} id={isNew ? "new-answer" : undefined}>
        <div className="question-comment-item">
          <div className="question-comment-header">
            ...
            <div className="question-comment-user">
              {answer.author?.first_name || answer.author?.username || 'Пользователь'}
            </div>
            <div className="question-comment-timestamp">
          ...
          <div className="question-comment-actions-right">
            <div 
              className={`question-reaction-badge ${answer.reactions?.user_reaction === 'like' ? 'question-reaction-badge--active' : ''}`}
              onClick={() => handleAnswerReaction('like')}
            ...
            <div 
              className={`question-reaction-badge ${answer.reactions?.user_reaction === 'dislike' ? 'question-reaction-badge--active' : ''}`}
              onClick={() => handleAnswerReaction('dislike')}
              style={{ cursor: 'pointer' }}
            >
          ...
          {isFirst ? (
            <div className="comment-actions-container">
              ...
              {answer.replies?.length > 0 && (
                ...
                >
                  {showReplies
                    ? `Скрыть комментарии (${answer.replies.length})`
                    : `Посмотреть ${answer.replies.length} Комментария`}
                </button>
              )}
            </div>
          ) : (
            answer.replies?.length > 0 && (
              <div className="question-comment-actions-left">
                ...
                >
                  {showReplies
                    ? `Скрыть ответы (${answer.replies.length})`
                    : `Показать ответы (${answer.replies.length})`}
                </button>
              </div>
            )
          )}
                    ...
                    <div className="question-subcomment-item">
                      ...
                      <div className="question-subcomment-body">
                        <div className="question-subcomment-header">
                          <div className="question-subcomment-user">
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          ...
                          <div className="question-subcomment-reactions">
                            <div 
                              className={`question-subcomment-reaction ${reply.reactions?.user_reaction === 'like' ? 'active' : ''}`}
                            ...
                            <div 
                              className={`question-subcomment-reaction ${reply.reactions?.user_reaction === 'dislike' ? 'active' : ''}`}
                        ...
                        <p style={{ color: '#1976D2', fontSize: '15px', fontFamily: 'Montserrat, sans-serif', margin: '0', fontWeight: '600' }}>
                          Ответить
                        </p>
                      </div>
                    </div>
