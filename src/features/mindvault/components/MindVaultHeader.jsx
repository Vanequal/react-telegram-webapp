...
import '@/styles/features/MindVaultHeader.scss'
import { RiArrowLeftSLine } from 'react-icons/ri'
import { useSelector } from 'react-redux'

const MindVaultHeader = ({ onDescriptionClick, onBackClick, hideSectionTitle, hideDescription, title, bgColor, textColor = '#1E88D3', fontSize = '24px' }) => {
    ...
    <>
      <section className="mind-vault-header" style={{ backgroundColor: bgColor || 'white' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <h1 className="mind-vault-header__title" style={{ color: textColor, fontSize }}>
            {title || localeTexts?.messages?.title || 'Копилка идей'}
          </h1>
          {!hideSectionTitle && (
            <p style={{ margin: 0, padding: 0, color: '#1E88D3' }}>
              {/* Используем заголовок из theme API */}
              {themeLoading ? 'Загрузка@.' : themeError ? 'Ошибка загрузки' : theme?.title || 'Заголовок раздела'}
            </p>
          )}
        </div>
      </section>
      ...
      {!hideDescription && (
        <p className="mind-vault-header__description" onClick={onDescriptionClick} style={{ cursor: 'pointer' }}>
