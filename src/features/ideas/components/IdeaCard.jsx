...
import CommentsList from '@/features/discussion/components/CommentsList'
...
// Styles
import '@/styles/features/idea-card.scss'
...
const IdeaCard = React.memo(function IdeaCard({ idea, onExpand, isExpanded = false, onCollapse, commentCount = 0, sectionCode, themeId }) {
  ...
  const currentLikes = currentPost?.reactions?.count_likes ?? currentPost?.likes ?? idea.likes ?? 0

  const currentDislikes = currentPost?.reactions?.count_dislikes ?? currentPost?.dislikes ?? idea.dislikes ?? 0
      ...
      <div className="idea-card" ref={cardRef}>
        ...
        <div className="idea-card__top">
          <div className="idea-card__user">
            ...
            <span className="idea-card__username">{idea.author?.username || idea.username || 'Аноним'}</span>
          </div>
        ...
        <div ref={textWrapperRef} className={`idea-card__text-wrapper ${expanded ? 'expanded' : ''}`}>
          <div className="idea-card__text-row">
            <div className="idea-card__text">{idea.text || idea.preview || idea.content || 'Нет текста'}</div>
          </div>
        </div>
        ...
        {ideaFiles?.length > 0 && (
          <div className="idea-card__files">
            <FileAttachments files={ideaFiles} onImageClick={handleImageClick} />
          </div>
        )}
        ...
        <div className="idea-card__actions-container">
          ...
          <span className="idea-card__timestamp">{formatTimestamp(idea.created_at || idea.timestamp)}</span>
        </div>

        <div className="idea-card__divider" />

        {/* Footer */}
        <div className="idea-card__footer" onClick={handleCardExpand} style={{ cursor: 'pointer' }}>
          {/* Показываем avatar stack только если есть комментарии */}
          {displayCommentCount > 0 && <img src={avatarStack} alt="Avatars" className="idea-card__avatar-stack" />}
          <span className="idea-card__comments">{displayCommentCount > 0 ? `${displayCommentCount} комментариев` : 'Прокомментировать'}</span>
          <img src={donatIcon} alt="Donate" className="idea-card__icon-donat" />
        </div>

        {/* Comments List (when expanded) */}
        {expanded && comments.length > 0 && <CommentsList comments={comments} />}
