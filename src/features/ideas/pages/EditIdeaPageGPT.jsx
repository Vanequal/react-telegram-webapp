...
const EditIdeaPageGPT = () => {
  ...
  const handlePublish = useCallback(
    async (text, publishing_method = 'original') => {
      if (!text || !text.trim()) {
      ...
      try {
        ...
        if (actionResult.meta.requestStatus === 'fulfilled') {
          ...
          // Обработка ошибок
          const errorMsg = typeof actionResult.payload === 'string' ? actionResult.payload : actionResult.payload?.message || actionResult.payload?.detail || 'Неизвестная ошибка'
  ...
  return (
    <div className="edit-idea-page-gpt">
      <MindVaultHeader onBackClick={handleNavigateBack} onDescriptionClick={() => {}} hideSectionTitle={true} hideDescription={true} textColor="black" bgColor="#EEEFF1" />
      ...
      <div className="edit-idea-page-gpt__content">
        ...
        {!preview ? (
          ...
          <>
            ...
            <PreviewActions onPublishOriginal={handlePublishOriginal} onPublishGPT={handlePublishGPT} onEditGPT={handleEditGPT} loading={loading} hasOriginal={!!preview.original_text} hasGPT={!!preview.gpt_text} />
          </>
        )}
      </div>
...
const PreviewActions = ({ onPublishOriginal, onPublishGPT, onEditGPT, loading, hasOriginal, hasGPT }) => (
  <div className="idea-card-gpt__actions">
    <button className="idea-card-gpt__action-button" onClick={onPublishOriginal} disabled={loading || !hasOriginal} title={!hasOriginal ? 'Оригинальный текст недоступен' : ''}>
      {loading ? 'Публикуем...' : 'Опубликовать оригинал'}
    </button>

    <button className="idea-card-gpt__action-button" onClick={onPublishGPT} disabled={loading || !hasGPT} title={!hasGPT ? 'GPT версия недоступна' : ''}>
      {loading ? 'Публикуем...' : 'Опубликовать версию GPT'}
    </button>

    <button className="idea-card-gpt__action-button idea-card-gpt__action-button--secondary" onClick={onEditGPT} disabled={loading || !hasGPT} title={!hasGPT ? 'GPT версия недоступна для редактирования' : ''}>
