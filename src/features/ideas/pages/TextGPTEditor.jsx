...
const TextGPTEditor = () => {
  ...
  return (
    <div className="text-gpt-editor">
      <MindVaultHeader title="Редактировать" onBackClick={handleBack} hideSectionTitle={true} hideDescription={true} bgColor={'#EEEFF1'} textColor="black" />

      <div className="text-gpt-editor__content">
        {/* Информация о файлах если есть */}
        {attachedFiles && attachedFiles.length > 0 && <div className="text-gpt-editor__files-info">📎 Прикреплено файлов: {attachedFiles.length}</div>}

        {/* Показываем ошибки */}
        {error && <div className="text-gpt-editor__error">❌ {error}</div>}

        <textarea className="text-gpt-editor__textarea" value={text} onChange={e => setText(e.target.value)} placeholder="Введите текст..." autoFocus disabled={isPublishing} />

        {/* Кнопки действий */}
        <div className="text-gpt-editor__actions">
          {hasChanges && <div className="text-gpt-editor__changes-indicator">✏️ Текст изменен</div>}

          <button className="text-gpt-editor__publish-btn" onClick={handlePublish} disabled={!canPublish || isPublishing || loading}>
